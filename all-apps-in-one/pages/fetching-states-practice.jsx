...
function Menubar({ userId }) {
  ...
  return (
    <nav className="fetching-states-nav">
...
function Labels() {
  ...
  return (
    <>
      <h2>Labels</h2>
      <ul>
        {labelsQuery.isLoading ? <p>...</p> : labels.map((label) => (
          <li key={label.id} className={`fetching-states-label ${label.color}`}>
            {label.name}
          </li>
        ))}
      </ul>
...
export default function FetchingStatesApp() {
  return (
    <div>
      <Menubar userId="u_2" />
      <main className="fetching-states-main">
        <section className="fetching-states-section">
          <Issues />
        </section>
        <aside className="fetching-states-aside">
          <Labels />
        </aside>
      </main>
    </div>
